...
likert_plot_odd <- function(data, title, questions, levels, n, levels_to_drop, positive_side = c("lower", "upper"), limits = c(-0.5, 1), text_push = 0.1, textsize = 30) {
  ...
  likert_plot_data <-
    ...
    mutate(levels_int = as.integer(.data[[levels]]))
  ...
  # Save color palette
  if(max(likert_plot_data$levels_int==7)){
    color_palette <- 
      data.frame(
        colors = c("#CD7F32", "#D5A16D", "#DDC3A9", "grey90", "#99D0CD", "#4EBCB5", "#03A89E"),
        levels_int = 1:7)
  } else if(max(likert_plot_data$levels_int==5)){
    color_palette <- 
      data.frame(
        colors = c("#D5A16D", "#DDC3A9", "grey90", "#99D0CD", "#4EBCB5"),
        levels_int = 1:5)
  } else if(max(likert_plot_data$levels_int==3)){
    color_palette <- 
      data.frame(
        colors = c( "#DDC3A9", "grey90", "#99D0CD"),
        levels_int = 1:3)
  }
  ...
  likert_plot_data <-
    likert_plot_data %>% 
    group_by(.data[[questions]]) %>%
    mutate(prop = .data[[n]] / sum(.data[[n]]),
           {{questions}} := as.factor(.data[[questions]]),
  ...
  label_data <-
    likert_plot_data %>%
    group_by(.data[[questions]], support) %>%
  ...
  likert_plot <-
    ...
    geom_bar(
      ...
      aes(x = .data[[questions]], y = prop, fill = colors),
      position = "stack",
      stat = "identity"
    )  +
    geom_bar(
  ...
  likert_plot <-
    ...
    geom_text(
      ...
      aes(label = label, x = .data[[questions]],  y = textloc),
      hjust = 0, color = "black", size = textsize*1/3) +
    geom_text(
      data = label_data$positive,
      aes(label = label, x = .data[[questions]],  y = textloc),
      hjust = 1, color = "black", size = textsize*1/3)  +
    geom_text(
      data = label_data$neutral,
      aes(label = label, x = .data[[questions]],  y = textloc),
      hjust = .5, color = "black", size = textsize*1/3) +
    scale_y_continuous(
      expand = c(0, 0),
      limits = limits,
      breaks = breaks,
