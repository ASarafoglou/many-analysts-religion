...
likert_plot_new <- function(dat, items, title, labs, levels, yaxis = TRUE, positive_side = c("lower", "upper"), 
                        limits = c(-1, 1.2), text_push = 0.1, textsize = 30, 
                        cols = c("#03A89E","#CD7F32")) {
  
  `%ni%` <- Negate(`%in%`)
  nlevels <- length(levels)
  nquestions <- length(items)
  
  # Counts per level of Likert scale
  n <- NULL
  
  for(i in 1:length(items)){
    n <- c(n, table(factor(dat[,items[i]], levels=1:nlevels)))
  }
  
  # Create dataframe with counts
  data <- data.frame(
    item = rep(factor(labs, levels = rev(labs)), each = nlevels),
    levels = rep(factor(levels, levels = levels), nquestions), 
    n = n)
